import { Product, StockBatch, Sale, Expense, Customer, StockStatus, UserRole, Supplier, SupplierPayment, User, CustomerPayment, CashAdjustment } from '../../types';
...
// In Dev (Vite), 'vite.config.ts' proxies '/api' -> 'http://localhost:3001/api'
// In Prod (Node), the server hosts both frontend and backend on the same origin.
const API_URL = '/api';
...
class ApiClient {
  ...
  async fetchAll() {
      try {
        const [users, prods, stock, sales, exp, cust, cp, supp, sp, adj] = await Promise.all([
            fetch(`${API_URL}/users`).then(r => r.json()),
            fetch(`${API_URL}/products`).then(r => r.json()),
            fetch(`${API_URL}/stock`).then(r => r.json()),
            fetch(`${API_URL}/sales`).then(r => r.json()),
            fetch(`${API_URL}/expenses`).then(r => r.json()),
            fetch(`${API_URL}/customers`).then(r => r.json()),
            fetch(`${API_URL}/customer-payments`).then(r => r.json()),
            fetch(`${API_URL}/suppliers`).then(r => r.json()),
            fetch(`${API_URL}/supplier-payments`).then(r => r.json()),
            fetch(`${API_URL}/cash-adjustments`).then(r => r.json()),
        ]);
  ...
  async addCustomerPayment(customerId: string, amount: number, createdBy: string) {
    const payment: any = {
